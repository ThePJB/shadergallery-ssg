...
float dLines(vec2 p, float t) {
    ...
    for (float i = 1.0; i < 12.0; i+=1.0) {
        vec2 p1 = abs(vec2(snoise(vec2(t, i*132.53)), snoise(vec2(t, i*159.41))));
        vec2 p2 = abs(vec2(snoise(vec2(t, i*21.7)), snoise(vec2(t, i*47.65))));
...
void main() {
    ...
    float thetat = triangle_mod(theta + time * 0.1, 2.0*PI/n_sides);
    ...
    float d1 = dLines(uv_k, time * 0.05);
    final_colour.x = 1.0-d1;
    final_colour.y = 1.0-d1*2.0;

    float d2 = dLines(uv_k, 123.71 + time * 0.05);

    if (d2 < 0.04) {
        final_colour = vec4(0.0, 0.0, 0.0, 1.0);
    } 
    if (d2 < 0.03) {
        final_colour = vec4(2.0*thetat, 0.5-r, r, 1.0);
    }
